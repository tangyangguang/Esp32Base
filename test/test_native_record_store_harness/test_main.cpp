...
Esp32BaseRecordStore::StoreDefinition definition(const char* name = "watering",
    ...
    value.maximumFileBytes = maximumBytes;
...
bool Esp32BaseFs::mkdir(const char* path) { g_directories.insert(path ? path : ""); return true; }
...
void test_create_calculates_capacity_and_file_size() {
    ...
    TEST_ASSERT_EQUAL_UINT32(3, status.capacity);
    TEST_ASSERT_EQUAL_UINT32(32, status.slotSizeBytes);
    TEST_ASSERT_EQUAL_UINT32(224, status.actualFileBytes);
    TEST_ASSERT_EQUAL_STRING("/esp32base/records/watering.v1.bin", status.path);
    TEST_ASSERT_EQUAL_UINT32(2, g_fixedSlotVisitCalls);
}

void test_append_rotates_and_reads_latest_first() {
    Esp32BaseRecordStore store;
    TEST_ASSERT_TRUE(store.begin(definition()));
    uint8_t bytes[8];
    ...
    TEST_ASSERT_EQUAL_UINT32(3, collected.ids.size());
    TEST_ASSERT_EQUAL_UINT32(5, collected.ids[0]);
    TEST_ASSERT_EQUAL_UINT32(4, collected.ids[1]);
    TEST_ASSERT_EQUAL_UINT32(3, collected.ids[2]);
    TEST_ASSERT_EQUAL_UINT32(3, g_fixedSlotVisitCalls);
}
...
void test_corrupt_record_is_skipped_and_reported() {
    ...
    uint8_t bytes[8]; payload(bytes, 1);
    TEST_ASSERT_TRUE(store.appendInstant(bytes, sizeof(bytes)));
    g_files[store.path()][128 + 20] ^= 0x55;
...
void test_different_versions_use_different_files() {
...
void test_existing_wrong_size_is_not_rewritten() {
    Esp32BaseRecordStore store;
    TEST_ASSERT_TRUE(store.begin(definition()));
    g_files[store.path()].resize(200);
    TEST_ASSERT_FALSE(store.reload());
    TEST_ASSERT_EQUAL(Esp32BaseRecordStore::StoreState::StructuralFault, store.state());
    TEST_ASSERT_EQUAL_UINT32(200, g_files[store.path()].size());
}

void test_existing_definition_mismatch_is_not_rewritten() {
    Esp32BaseRecordStore store;
    TEST_ASSERT_TRUE(store.begin(definition()));
    std::vector<uint8_t>& file = g_files[store.path()];
    for (uint32_t copy = 0; copy < 2; ++copy) {
        ...
        recordStoreWriteU32(header + 36, recordStoreCrc(header, 64));
    }
    ...
    TEST_ASSERT_EQUAL_UINT32(beforeReload.size(), g_files[store.path()].size());
    TEST_ASSERT_EQUAL_UINT8_ARRAY(beforeReload.data(), g_files[store.path()].data(), beforeReload.size());
}
...
void test_minimum_free_space_blocks_business_store_creation() {
    ...
    TEST_ASSERT_FALSE(Esp32BaseFs::exists("/esp32base/records/door-opening.v1.bin"));
}

void test_partial_slot_writes_never_return_incomplete_new_record() {
    Esp32BaseRecordStore store;
    TEST_ASSERT_TRUE(store.begin(definition("feeding", 1, 160)));
    uint8_t bytes[8]; payload(bytes, 1);
    TEST_ASSERT_TRUE(store.appendInstant(bytes, sizeof(bytes)));
    const std::string path = store.path();
    const std::vector<uint8_t> oldSlot(g_files[path].begin() + 128, g_files[path].begin() + 160);

    payload(bytes, 2);
    ++g_time.uptimeSec;
    TEST_ASSERT_TRUE(store.appendInstant(bytes, sizeof(bytes)));
    const std::vector<uint8_t> newSlot(g_files[path].begin() + 128, g_files[path].begin() + 160);

    for (size_t written = 1; written < newSlot.size(); ++written) {
        std::copy(oldSlot.begin(), oldSlot.end(), g_files[path].begin() + 128);
        std::copy(newSlot.begin(), newSlot.begin() + written, g_files[path].begin() + 128);
        TEST_ASSERT_TRUE(store.reload());
        Esp32BaseRecordStore::StoreStatus status;
        TEST_ASSERT_TRUE(store.readStatus(status));
        ...
        Esp32BaseRecordStore::RecordMetadata record;
        uint8_t output[8];
        TEST_ASSERT_NOT_EQUAL(Esp32BaseRecordStore::RecordReadResult::Found,
...
void test_app_events_uses_compact_payload_and_default_budget() {
    ...
    TEST_ASSERT_EQUAL_UINT32(2130, Esp32BaseAppEvents::capacity());
    TEST_ASSERT_EQUAL_STRING("/esp32base/records/app-events.v1.bin", Esp32BaseAppEvents::path());
    ...
    TEST_ASSERT_EQUAL_UINT32(102368, status.storage.actualFileBytes);
}

int main(int argc, char** argv) {
    UNITY_BEGIN();
    RUN_TEST(test_create_calculates_capacity_and_file_size);
    RUN_TEST(test_append_rotates_and_reads_latest_first);
    ...
    RUN_TEST(test_different_versions_use_different_files);
    RUN_TEST(test_existing_wrong_size_is_not_rewritten);
    ...
    RUN_TEST(test_partial_slot_writes_never_return_incomplete_new_record);
