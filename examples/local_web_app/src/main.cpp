...
#if defined(ESP32BASE_LOCAL_WEB_APP_WIFI_SSID) && defined(ESP32BASE_LOCAL_WEB_APP_WIFI_PASS)
#define ESP32BASE_LOCAL_WEB_APP_HAS_WIFI_CREDENTIALS 1
#else
#define ESP32BASE_LOCAL_WEB_APP_HAS_WIFI_CREDENTIALS 0
#endif

void handleHeadExtra() {
    Esp32BaseWeb::sendChunk("<style id='web-app-head-extra'>nav a.active{font-weight:700}</style>");
}
...
void handleControlApi() {
    ...
    if (!Esp32BaseWeb::checkPostAllowed("local_web_app_control")) {
        return;
    }
...
void handleCsvApi() {
    ...
    char value[64] = "";
    Esp32BaseConfig::getStr(APP_NS, APP_KEY_VALUE, value, sizeof(value), "");
    if (!Esp32BaseWeb::beginCsv(200, "local_web_app.csv")) {
        return;
    }
...
void setup() {
    Esp32Base::setFirmwareInfo("local-web-app", "1.0.0");
    Esp32BaseWeb::setDefaultAuth("admin", "admin");
    Esp32BaseWeb::setDeviceName("Local Web App");
    ...
    Esp32Base::begin();
#if ESP32BASE_LOCAL_WEB_APP_HAS_WIFI_CREDENTIALS
    Esp32BaseWiFi::connect(ESP32BASE_LOCAL_WEB_APP_WIFI_SSID, ESP32BASE_LOCAL_WEB_APP_WIFI_PASS, true);
#endif
    ...
    ESP32BASE_LOG_I("example", "local web app ready");
}
