# MQTT Client (MQTTS) Example

This example uses the `IOT` Profile, which combines the local Web/OTA maintenance plane with the MQTT 3.1.1 client. MQTT is provided by the profile itself; the build does not set `ESP32BASE_ENABLE_MQTT` separately.
...
python3 scripts/pio_arduino.py 2 run -d examples/mqtt_client
python3 scripts/pio_arduino.py 2 run -d examples/mqtt_client -e esp32s3
python3 scripts/pio_arduino.py 2 run -d examples/mqtt_client -e esp32c3
python3 scripts/pio_arduino.py 3 run -d examples/mqtt_client -e esp32_arduino3
```
