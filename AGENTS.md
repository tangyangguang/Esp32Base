...
- 同一模块、同一风险范围内的相关修改合并验证，测试必须能说明所覆盖的风险；纯文档修改只做文档、链接和 diff 检查。跨芯片/Core 检查仅在本次确实影响该目标时选择必要组合，正式发布的范围另行确定，禁止每个小改动重复全量测试。只有新修改、新失败或证据疑点使原结果失效时才重跑，不能为省时间取消必要验证，也不能用无关测试消耗时间和 Token。
- 验证顺序以稳健推进主线和总成本最低为准：先依据当前应用配置确定主用芯片/Core，开发期间集中完成主用组合的功能、失败路径与必要构建；其他 Core、ESP32-S3/ESP32-C3 等非主用组合不自动进入当前任务的验证范围，不阻塞无关主线，也不提前宣称全部兼容。若当前改动直接涉及某组合独有的 API、ABI、安全能力或依赖，才提前验证该高风险组合；不能为了只测一个版本降低安全要求。
