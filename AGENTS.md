...
- `Esp32Base` 与 `Esp8266Base` 永久独立：任何时候都不得合库、互相依赖、建立共同 Base 或跨芯片 HAL，也不得为了接口一致复制对方内部架构。可借鉴对方的故障证据和方案，但必须结合本库实际需求、芯片及运行模型独立评审。
- 平台专属 Topic、消息信封、连接周期编码、诊断上报和记录确认流程由独立平台 SDK 承担；依赖方向只能是应用/平台 SDK -> Esp32Base。本库不得增加某个 IoT 平台的模式开关或依赖其定义、协议实现；普通 LOCAL 应用无需引用平台 SDK。
...
- 优化以可靠复用和降低业务接入负担为目标，优先减少重复计算、无效分配、重复状态及资源峰值叠加；不得为节省 RAM/Flash 削弱 WiFi 重连、MQTT、TLS、Watchdog、关键持久化的必要能力和安全余量。评估 Base + SDK + 应用的总成本，不能把开销移给上层后声称节省。
- 必需的正确方案不能因构建、维护或验证麻烦而省略；同时不为未确认需求增加能力。明确资源、工具链和维护影响，在已确认范围内完成；出现新的实质取舍时先沟通，不用关闭校验或隐式降级掩盖问题。
- 证书能力按实际通信场景和安全要求决定是否启用；需要时必须评估信任来源、身份与有效期校验、时间依赖、更新和失效恢复，并说明资源及维护代价。不得为了当前省事采用会把必要维护留给业务项目或以后返工的方案；不需要证书的能力不得被强制带入相关依赖。
...
- 大型改造拆成可独立验收的阶段任务，每阶段只围绕一个明确结果，携带目标、非目标、责任边界、当前事实和退出条件。实施前再次复审方案；阶段结束时报告实际结果、证据和剩余工作，不横向铺开多个未闭环改造。
...
- 在已授权目标和任务边界内，实施前复核必要性、合理性与依据；有充分把握的变更直接推进，不因与原方案存在局部差异而反复请求确认。关键判断不确定，或变更超出已授权边界、需要新的用户取舍时，暂停受影响部分，说明问题、依据、影响和建议，沟通明确后继续；不受影响的工作可继续。不得带疑实施、擅自扩大范围或降低已确认的可靠性与安全要求。
- 每次任务或阶段总结必须明确列出实际变更及所属模块或仓库；相对原方案有调整时，单独说明“原计划是什么、实际改成什么、为什么调整、影响及验证结果”，不得只报告完成状态或提交号。已自主实施的必要变更也必须披露，并列明未验证边界和剩余工作。
...
- Web、OTA、FS、FileLog、Record Store、Conditions、App Config、RTC、RS485 等可选能力必须保持可裁剪。能力关闭时，不得编译或链接专属重依赖，不得初始化、注册静态对象或保留专属页面资源；不要只以“运行时未调用”代替裁剪证明。
...
- 同一模块、同一风险范围内的相关修改合并验证，测试必须能说明所覆盖的风险；纯文档修改只做文档、链接和 diff 检查。全量/跨芯片/Core 矩阵在影响该边界的阶段末或发布时集中执行，禁止每个小改动重复全量测试。只有新修改、新失败或证据疑点使原结果失效时才重跑，不能为省时间取消必要验证，也不能用无关测试消耗时间和 Token。
- 验证顺序以稳健推进主线和总成本最低为准：先依据当前应用配置确定主用芯片/Core，开发期间集中完成主用组合的功能、失败路径与必要构建；其他 Core、ESP32-S3/ESP32-C3 等非主用组合在相关功能收齐后的统一兼容性验收中集中验证、修复，不阻塞无关主线，也不提前宣称全部兼容。若当前改动直接涉及某组合独有的 API、ABI、安全能力或依赖，才提前验证该高风险组合；不能为了只测一个版本降低安全要求。
- 耗时的 SDK/工具链重建不作为日常代码验证：优先复用来源、配置、目标和哈希均匹配的已验证产物及增量缓存；只有相关输入改变或证据失效时才重建。开始重型任务前判断其是否阻塞当前目标，非阻塞任务后置；日志保存到文件，仅检查结果和必要诊断，避免反复全量读取、无意义轮询和逐项停顿。
...
- 仓库保留当前契约、必要使用文档和结果性验证证据，不新增长期保留的过程计划、讨论稿或阶段流水账。确认已被当前契约取代且无有效引用的废弃过程稿按用户授权清理；未知用途文件、用户工作和经验材料不得混作过程稿删除。
