...
- FS 上传完成后会清理请求级上传状态；没有 multipart 文件的上传 POST 会返回 `No upload received`，不会复用上一笔上传路径。App Events store 或 FileLog 路径上传后如果运行态 reload 失败，会向维护页面返回明确错误，不再显示伪成功；如果上传已经截断/修改这些运行态敏感文件但随后失败或中止，也会先刷新运行态，避免内存继续沿用旧 head/count 或文件大小。
- FS 上传在发起目标截断/创建尝试前就记录敏感运行态恢复需求，避免底层写入返回失败但 LittleFS 已有创建或截断副作用时漏掉 App Events/FileLog reload；删除 App Events store 后，App Events reload/clear 也会独立于 FileLog fault 恢复结果执行，避免 FileLog 恢复失败阻断 App Events 运行态清理。
- 静态检查脚本补充上传截断失败恢复语义、删除后 App Events/FileLog reload 顺序，以及 `WebInternal.h`/`WebFs.cpp` 重点 helper 签名一致性校验。
...
- `Esp32BaseFileLog` 在业务接入文档和内置导航中统一定位为 System Diagnostic Logs / 系统诊断日志；默认 Web 标签从 `Logs` 调整为 `System Logs`，`/esp32base/logs`、`BUILTIN_LOGS` 和 `Esp32BaseFileLog` API 名称不变。
- App Events 也不是第二套系统诊断日志。业务项目不要把 boot/reset/restart reason、WiFi、NTP、OTA、LittleFS mount/write fault、FileLog fault、基础库健康状态等 Esp32Base 系统事件写入 App Events；同一底层故障可同时留下系统诊断日志和 App Event，但二者处于不同语义层：系统诊断日志写技术事实和内部错误链路，App Event 写业务影响、保护动作、跳过原因、用户维护结果或外部决策结果。
