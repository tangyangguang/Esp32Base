...
Status 页不创建后台任务、定时器、历史采样缓存或自动刷新。默认视图只读取已有内存状态和常数级平台指标；当前镜像大小每次启动最多读取一次镜像元数据并缓存。分区遍历、NVS 统计、网络参数与摘要等低频读取仅在请求 `?details=1` 时执行，因此页面关闭后没有持续 CPU、RAM 分配或 Flash 写入成本。
