...
- `/esp32base` Status 页是只读设备体检页，采用诊断优先结构：`System Overview` 首屏汇总设备/hostname、固件/profile、uptime/boot count、WiFi/IP/RSSI、free/min heap、NTP、FS、FileLog 和 OTA upload 空间；详细区再按 Runtime Health、Network、Storage & Logs、Firmware & OTA、Hardware、Partition Table 展示 heap、max alloc、Watchdog lifetime/trip resets、last reset/wake、MAC、OTA slot minus current sketch 和运行时分区表；页面容量值只显示 KB/MB/B 人性化格式，Max OTA upload 才是上传硬上限。
