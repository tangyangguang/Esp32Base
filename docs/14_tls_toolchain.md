...
Arduino Core 3.3.8 的官方预编译库未开启证书有效期校验。受控产物使用原包记录的源码版本、组件锁和配置，通过 Espressif Arduino Library Builder 重新构建，启用 `CONFIG_MBEDTLS_HAVE_TIME_DATE=y`，并仅在 ESP32 产物中将 MQTT task 固定到 Core 0。不通过修改头文件、替换单个函数或关闭校验绕过限制。
...
配置变化由来源锁明确列出：所有目标仅开启日期校验；ESP32 另外开启 `CONFIG_MQTT_TASK_CORE_SELECTION_ENABLED=y`、选择 `CONFIG_MQTT_USE_CORE_0=y`，并保持 `CONFIG_MQTT_USE_CORE_1=n`。后两项属于同一核选择；S3/C3 暂不添加此配置。网络缓冲、任务栈、优先级、重连及其他未列出的配置发生变化时，审计失败；组件锁变化也直接失败。源码只允许工具明确生成的配置补充和下面的构建依赖调整。
...
前者验证同版上游 X.509 源码的正常、过期、尚未生效、错误 hostname、不受信任签发者五种情况，并检查具体错误标志。后者只使用公开示例占位配置，核对包的来源及文件哈希，要求 SDK 头文件实际开启日期校验及 ESP32 MQTT Core0配置，并检查最终 ELF 中的证书链验证和日期路径。两者均不连接设备或 Broker。
...
受控 Core 3.3.8 / ESP32 新产物相对官方预编译包的完整配置差异仅为：日期校验开启、MQTT task 核选择开启、选择 core 0 且不选择 core 1；组件锁不变。QIO/DIO 两套配置头及证书链验证器对日期路径的实际调用通过审计。构建保留了上游组件的编译警告及旧 Kconfig 符号提示，没有为消除提示修改上游代码；不能声称 SDK 零警告。

七项工具链审计回归通过，覆盖无关内存配置变化、错误核、未固定核、日期关闭、依赖漂移及目标范围。五类同版主机证书用例通过。公开示例及实验板的新产物验证结果见 [统一验证结果](15_validation_results.md)，静态体积也统一在该文档维护。
...
当前日期校验 + ESP32 MQTT core0 产物已固定在 `local_private/toolchains/esp32-core-3.3.8-tls-9033b58a578a5b78/`。构建凭据 `esp32base-build.json` 的 SHA256 为 `9033b58a578a5b78c0316f3ade031a0ba22b8ba2a0f59e8d6fe61a48e5ef6e28`；复制前后核对来源锁、构建脚本哈希及全部3,795个文件哈希。应用可将上文 `file://` 路径指向该固定目录。该目录不纳入源码 Git 或基础库发布包，清理构建缓存不得连带清理它。

旧目录 `local_private/toolchains/esp32-core-3.3.8-tls-6da95a99041ac119/` 只含日期校验，不含本次核分配，保留用于历史对照和回退；不能用旧目录证明 core0 行为。
