...
`examples/record_store` 使用保护模式，以 Serial 输出演示本地消费，每32条消费一次检查点；重启可能重复输出。它逐条读取并在缺号/损坏时停止，不把 MQTT 入队、PUBACK 或打印行为描述为平台提交确认。已有旧示例 Store 使用默认轮转策略时会因定义不匹配被拒绝，需按升级规则明确处理旧数据，不能自动清空。
