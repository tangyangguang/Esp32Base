...
- `sendChunk()` / `sendProgmem()` 共用 512 B 静态 chunk buffer，但单个文本 chunk payload 上限为 128 B，并通过 Arduino `WebServer::sendContent()` 输出 chunked response；这样保留框架的 chunked 编码和连接收尾语义，同时避开 Arduino ESP32 Core 2.x 在 511 B payload 边界后续写入处偶发卡住的问题。
