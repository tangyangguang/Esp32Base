...
- 启用 Watchdog 的 profile 显示 Watchdog trip 小计维护；Reset Watchdog Trip 只写入 `eb_sys.wdt_trip_base` 和 `eb_sys.wdt_trip_time`，不清 `eb_sys.wdt_cnt`。
...
- Runtime Health 显示 heap free/min/max alloc/total；启用 Watchdog 时显示 Watchdog enabled、lifetime resets、trip resets 和 trip reset time。
