...
- 需要确认正常离线的产品先 `beginShutdown()`，匹配最终消息PUBACK及本地断开事件后才报告成功；覆盖入队失败、掉线、PUBACK/断开超时、邮箱丢失、恢复和维护衔接。未确认PUBACK的受控失败不发送正常DISCONNECT，OTA失败释放准备资源并恢复，强制安全重启/休眠不被阻塞。普通 `setBeforeNetworkStopCallback()` 宽限仍只是尽力发送，不冒充确认；Broker没有DISCONNECT ACK，异常掉电LWT及真实链路另验。
