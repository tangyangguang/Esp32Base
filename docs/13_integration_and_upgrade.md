...
- 使用 `symlink://` 引用独立、唯一的 Esp32Base Git 工作区；不要在业务仓库内复制源码、维护 `third_party/Esp32Base`、Git submodule 或私有修改版。
- 更新或适配业务项目时，先把统一 Esp32Base 工作区更新到当前最新提交；业务需要的共用能力应在本库实现、验证并提交，不在业务项目中分叉实现。
- 每次正式构建或发布由业务项目在自己的构建或验收记录中写明实际使用的 Esp32Base Git 提交。基础库工作区存在未提交修改时，应明确记录该构建不可由提交号完整复现。
- 复现历史构建时，根据记录临时检出对应 Esp32Base 提交；复现结束后恢复当前版本，不为历史构建保留源码副本。

本仓库只提供接入规范和[已知使用项目清单](../CONSUMERS.md)；具体引用方式、版本记录和升级时机由各业务项目负责。
