...
- 当底层 `FS used` 明显大于可见文件合计时，应明确提示内部/历史占用异常；不要让用户误以为文件列表漏算。删除可见文件不一定释放全部空间，必要时应备份重要文件后格式化 LittleFS。
- 文件系统详情页默认保持只读；需要维护时只能进入显式管理模式，提供单文件删除，不做批量删除、目录删除、下载、编辑或通用文件管理器。
