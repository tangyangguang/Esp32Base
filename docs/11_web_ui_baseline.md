...
- 文件系统详情页默认保持只读；只读模式允许单文件下载。文件声明有大小但首块无法读取时，必须标记 `unreadable`，不能给用户一个会下载出 0 字节文件的成功入口。需要维护时只能进入显式管理模式，提供单文件删除和受限上传，不做批量删除、目录删除、编辑、重命名、移动或通用文件管理器。上传保留本地文件名，只允许选择已有目录，不创建目录；`/logs` 等基础库日志目录不能作为上传目标。
