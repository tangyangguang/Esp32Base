...
for capability in (
    ...
    expect_failure(
        {capability: "2"},
        "All ESP32BASE_ENABLE_* capability macros must be 0 or 1",
    )

for option in (
    "ESP32BASE_WEB_ALLOW_INSECURE_DEFAULT_AUTH",
    "ESP32BASE_STRICT_OPTIONAL_BEGIN",
    "ESP32BASE_OTA_REQUIRE_MARK_VALID",
):
    expect_failure({option: "2"}, f"{option} must be 0 or 1")

expect_failure(
    {"ESP32BASE_OTA_MARK_VALID_TIMEOUT_MS": "0"},
    "ESP32BASE_OTA_MARK_VALID_TIMEOUT_MS must be > 0",
)
expect_failure(
    {"ESP32BASE_ENABLE_NTP": "1", "ESP32BASE_ENABLE_TIME": "0", "ESP32BASE_ENABLE_WIFI": "1"},
    "ESP32BASE_ENABLE_NTP requires ESP32BASE_ENABLE_TIME",
)
expect_failure(
    {"ESP32BASE_ENABLE_APP_CONFIG": "1"},
    "ESP32BASE_ENABLE_APP_CONFIG requires ESP32BASE_ENABLE_WEB",
)
expect_failure(
    {"ESP32BASE_RECORD_STORE_TOTAL_MAX_BYTES": "207UL"},
    "ESP32BASE_RECORD_STORE_TOTAL_MAX_BYTES is too small for a Record Store",
)
expect_failure(
    {"ESP32BASE_FS_MINIMUM_SAFETY_RESERVE_BYTES": "4095UL"},
    "ESP32BASE_FS_MINIMUM_SAFETY_RESERVE_BYTES must be at least one 4 KiB flash block",
)
