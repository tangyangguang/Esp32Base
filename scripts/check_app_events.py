...
require("README.md", "FileLog/Status/System diagnostics 记录系统底层发生了什么", "README must separate App Events from system logs")
require("README.md", "App Event 写业务含义和用户可理解结果", "README must allow dual recording with separate meanings")
...
require("CHANGELOG.md", "不是第二套系统 FileLog", "CHANGELOG must warn business apps not to duplicate system logs")
require("CHANGELOG.md", "FileLog 写技术原因和内部错误链路", "CHANGELOG must clarify FileLog/App Events split")
...
require("docs/03_api.md", "应用业务为什么做了某个决定", "API docs must define FileLog vs App Events decision rule")
...
require("docs/07_diagnostics.md", "App Event 必须表达业务含义", "diagnostics docs must verify dual recording semantics")
require("docs/10_known_limitations.md", "App Events 边界", "known limitations must document App Events boundary")
require("docs/10_known_limitations.md", "不是第二套系统 FileLog", "known limitations must separate App Events from FileLog")
